...
# Aligned to TF Cloud variable names in your screenshot
########################################
...
########################################
# VARIABLES (names match your workspace)
########################################
...
# Required AMI: prefer fw_ami_id; keep pan_ami_id for compatibility
variable "fw_ami_id" {
  type        = string
  default     = ""
...
variable "fw_key_name"      { type = string, default = "" } # "acme admin key" in workspace
...
variable "tgw_id" { type = string, default = "" } # not used yet, reserved for external TGW
...
# Enforce that at least one AMI is provided
resource "null_resource" "check_ami" {
  lifecycle { precondition {
    condition     = length(local.effective_ami) > 0
    error_message = "Set fw_ami_id (preferred) or pan_ami_id to a valid AMI ID (ami-xxxxxxxx)."
  } }
}
...
resource "aws_route" "mgmt_default"  { route_table_id = aws_route_table.mgmt_rt.id  destination_cidr_block = "0.0.0.0/0"  gateway_id = aws_internet_gateway.fw_igw.id }
resource "aws_route_table_association" "mgmt_assoc" { route_table_id = aws_route_table.mgmt_rt.id  subnet_id = aws_subnet.fw_mgmt_1.id }

resource "aws_route_table" "untrust_rt" { vpc_id = aws_vpc.fw_vpc.id  tags = { Name = "${var.name_prefix}-fw-untrust-rt" } }
resource "aws_route" "untrust_default"  { route_table_id = aws_route_table.untrust_rt.id  destination_cidr_block = "0.0.0.0/0"  gateway_id = aws_internet_gateway.fw_igw.id }
resource "aws_route_table_association" "untrust_assoc" { route_table_id = aws_route_table.untrust_rt.id  subnet_id = aws_subnet.fw_untrust_1.id }

resource "aws_route_table" "trust_rt" { vpc_id = aws_vpc.fw_vpc.id  tags = { Name = "${var.name_prefix}-fw-trust-rt" } }
resource "aws_route_table_association" "trust_assoc" { route_table_id = aws_route_table.trust_rt.id  subnet_id = aws_subnet.fw_trust_1.id }
...
# Compute user_data from explicit input or S3 bootstrap toggle
locals {
  computed_user_data = (
...
########################################
# FLOW LOGS + S3 (conditional; sanitizes bad names)
########################################
resource "random_id" "suffix" { byte_length = 3 }

# If you typed "acme-sandbox logs-usw2 poc", we sanitize to valid DNS
locals {
...
resource "aws_s3_bucket" "logs" {
  ...
  bucket        = local.sanitized_logs_bucket
  force_destroy = true
...
########################################
# Create a new TGW for POC. If you later want to use an existing TGW (tgw_id),
# switch attachments to it and delete this resource.
