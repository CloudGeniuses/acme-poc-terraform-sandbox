...
# VM-Series with S3 bootstrap for day-0 credentials
########################################
...
variable "az_primary" {
  type        = string
  description = "Primary AZ letter"
  default     = "a"
}

variable "az_secondary" {
  type        = string
  description = "Secondary AZ letter"
  default     = "b"
}

variable "enable_ha" {
  type        = bool
  description = "Deploy a second firewall for HA topology"
  default     = true
}

variable "admin_cidr" {
  type        = string
  description = "CIDR allowed to originate to mgmt (22/443). Use a narrow office/VPN CIDR in real envs."
  default     = "0.0.0.0/0"
}
...
variable "fw_key_name" {
  type        = string
  description = "Optional EC2 key pair name (SSM bastion used typically)"
  default     = ""
}
...
variable "enable_s3_vpc_endpoint" {
  type        = bool
  description = "Add S3 gateway endpoint to route tables (optional)"
  default     = false
}

# --- Day-0 credentials (lab/demo). For production, use a safer mechanism.
variable "admin_username" {
  type        = string
  description = "Initial PAN-OS admin username"
  default     = "admin"
}

variable "admin_password" {
  type        = string
  description = "Initial PAN-OS admin password (meets PAN-OS complexity)"
  default     = "Admin2025!"
  sensitive   = true
}

# --- Legacy vars (declared only to silence existing TFC workspace warnings)
variable "enable_s3_bootstrap" {
  type        = bool
  description = "DEPRECATED (ignored) – bootstrap is always enabled."
  default     = true
}

variable "bootstrap_s3_prefix" {
  type        = string
  description = "DEPRECATED (ignored) – prefix fixed internally to 'bootstrap'."
  default     = ""
}

variable "bootstrap_s3_bucket" {
  type        = string
  description = "DEPRECATED (ignored) – module creates its own unique bucket."
  default     = ""
}
...
locals {
  ...
  # Heredoc cannot be used inline in a conditional; build as a normal string.
  user_data = "vmseries-bootstrap-aws-s3bucket=${local.bootstrap_bucket_name}\nvmseries-bootstrap-aws-s3prefix=${local.bootstrap_prefix}\n"
}
...
# Upload bootstrap.xml with admin creds (Day-0)
...
output "fw1_public_ip" {
  ...
  description = "Firewall 1 public EIP on untrust"
}

output "fw2_public_ip" {
  value       = var.enable_ha ? aws_eip.fw2_eip[0].public_ip : null
  description = "Firewall 2 public EIP on untrust (if HA)"
}
